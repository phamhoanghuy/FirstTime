<div class="row">
    <div class="col-md-12 col-lg-12 col-xs-12">
        <p class="service-title">Dịch vụ của chúng tôi</p>
        <p class="service-content">
            Giúp bạn quảng bá thương hiệu mạnh mẽ, tăng nhanh doanh số
        </p>
    </div>
    <div class="service-items col-md-12 col-lg-12 col-xs-12">
        <div class="service-item col-md-4 col-lg-4 col-xs-4">
            <div class="col-md-12 col-lg-12 col-xs-12">
                <img class="service-item-icon" src="{{ URL::to('images/service-icon-1.jpg') }}" alt="">
            </div>
            <div class="col-md-12 col-lg-12 col-xs-12">
                <p class="service-item-title">THIẾT KẾ</p>
                <hr style="width: 35%; border-bottom: 1px solid #eee; margin-left: 33%">
            </div>
            <div class="service-item-content col-md-12 col-lg-12 col-xs-12">
                <p class="service-item-content">Thiết kế các sản phẩm...</p>
            </div>
        </div>
        <div class="service-item col-md-4 col-lg-4 col-xs-4">
            <div class="col-md-12 col-lg-12 col-xs-12">
                <img class="service-item-icon" src="{{ URL::to('images/service-icon-2.jpg') }}" alt="">
            </div>
            <div class="col-md-12 col-lg-12 col-xs-12">
                <p class="service-item-title">IN ẤN</p>
                <hr style="width: 35%; border-bottom: 1px solid #eee; margin-left: 33%">
            </div>
            <div class="service-item-content col-md-12 col-lg-12 col-xs-12">
                <p class="service-item-content">In ấn trên các chất liệu...</p>
            </div>
        </div>
        <div class="service-item col-md-4 col-lg-4 col-xs-4">
            <div class="col-md-12 col-lg-12 col-xs-12">
                <img class="service-item-icon" src="{{ URL::to('images/service-icon-2.jpg') }}" alt="">
            </div>
            <div class="col-md-12 col-lg-12 col-xs-12">
                <p class="service-item-title">THI CÔNG</p>
                <hr style="width: 35%; border-bottom: 1px solid #eee; margin-left: 33%">
            </div>
            <div class="service-item-content col-md-12 col-lg-12 col-xs-12">
                <p class="service-item-content">Thi công các dự án...</p>
            </div>
        </div>
    </div>
</div>